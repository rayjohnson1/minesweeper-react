...
export default class GameController {
    ...
    initNeighbors(){
        ...
        for(let tileId in this._gameBoardList){
            ...
            if(tile.matrixCoordinate.y - 1 > (-1) && tile.matrixCoordinate.x + 1 < this._gameBoardMatrix.length){
                const neighbor = this._gameBoardMatrix[tile.matrixCoordinate.x + 1][tile.matrixCoordinate.y - 1];
                tile.neighbors[neighbor.id] = neighbor;
            }

            if(tile.matrixCoordinate.y + 1 < this._gameBoardMatrix.length && tile.matrixCoordinate.x - 1 > (-1)){
                const neighbor = this._gameBoardMatrix[tile.matrixCoordinate.x - 1][tile.matrixCoordinate.y + 1];
                tile.neighbors[neighbor.id] = neighbor;
            }
